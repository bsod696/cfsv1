                            ...
                            <div class="col-md-12 offset-md-0">
                                    <p>
                                        <form action="{{ route('staff.submit.cancelorder') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                            <input id="id" type="text" class="form-control{{ $errors->has('id') ? ' is-invalid' : '' }}" name="id"  value="{{ $updata['orders']->id }}" hidden />
                                            <button type="submit" name="submit" class="btn btn-primary">{{ __('Cancel Order') }}</button>
                                        </form>
                                    </p>
                                    <p> <b> Order Reference : </b>{{ $updata['orders']->id }}  </p>
                                    <p> <b> Parent Reference: </b>{{ $updata['orders']->parentid }} </p>
                                    <p> <b> Student Name: </b>{{ ucfirst($updata['orders']->studentname) }} </p>
                                    <br>

                                    <p> <b> Transaction ID : </b>{{ $updata['orders']->txid }} </p>
                                    <p> <b> Transaction Amount : </b>RM {{ number_format($updata['orders']->menuprice*$updata['orders']->menuqty, 2, '.', '') }} </p>
                                    <br>
                                   
                                    <p> <b> Menu Name: </b>{{ ucfirst($updata['menus']->menuname) }} </p>
                                    <p> 
                                        <img align="top" src="{{ $updata['menus']->menupic }}" width="240" height="160" border="1"> 
                                    </p>
                                    <p> <b> Menu Description: </b>{{ $updata['menus']->menudesc }} </p>
                                    <p> <b> Menu Type: </b>
                                    ...
                                    <p> <b> Menu Price: </b>RM {{ $updata['menus']->menuprice }} </p>
                                    <p> <b> Menu Quantity: </b>{{ $updata['orders']->menuqty }} Units </p>
                                    <p> <b> Menu Calories: </b>{{ $updata['menus']->menucalories }} KCal</p>
