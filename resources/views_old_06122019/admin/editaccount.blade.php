                ...
                <div class="card-body">
                    @foreach( $updata as $u)
                     <form action="{{ route('admin.submit.deleteaccount') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <button type="submit" name="submit" class="btn btn-primary">{{ __('Delete Account Detail') }}</button>
                    </form>
                    <form method="POST" action="{{ route('admin.submit.editaccount') }}">
                        @csrf
                        <input id="id" type="text" class="form-control{{ $errors->has('id') ? ' is-invalid' : '' }}" name="id"  value="{{ $u->id }}" hidden />
                        <input id="staffid" type="text" class="form-control @error('staffid') is-invalid @enderror" name="staffid" value="{{ $u->staffid }}" readonly hidden>
